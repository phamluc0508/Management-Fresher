...
public interface EmployeeRepo extends JpaRepository<Employee, Long> {
    ...
    @Query(
            value = "select e.id as id" + " , e.firstName as fistName"
    ...
    @Query(
            value = "select DISTINCT e" + " from Employee e"
                    ...
                    + " and ((lower(concat(coalesce(e.firstName,''), coalesce(e.middleName,''), coalesce(e.lastName,'') ))"
                    + " like lower(concat('%',:name,'%')) or coalesce(:name, '#') = '#'))")
    Page<Employee> searchEmployeeByCenterIds(
            @Param("name") String name,
            @Param("email") String email,
            @Param("position") String position,
            @Param("programmingLanguage") String programmingLanguage,
    ...
    @Query(
            value = "select a.assessmentType as type" + " , af.point as point"
                    ...
                    + " group by a.assessmentType, af.point, e"
                    + " order by a.assessmentType, af.point")
    List<Map<String, Object>> findEmployeesByPointAndCenterIds(
            @Param("centerIds") List<Long> centerIds, @Param("position") String position);

    @Query(
            value = "select e as employees" + " , AVG(af.point) as point"
                    + " , count(e.id) as quantity"
                    + " from Employee e"
                    + " inner join EmployeeCenter ec on e.id = ec.employeeId"
                    + " inner join AssessmentFresher af on e.id = af.employeeId"
    ...
    @Query(
            value = "select e as employees" + " , AVG(af.point) as point"
                    ...
                    + " having count(distinct a.assessmentType) = 3"
                    + " order by AVG(af.point)")
    List<Map<String, Object>> findEmployeesByAVGAndCenterIds(
            @Param("centerIds") List<Long> centerIds, @Param("position") String position);

    @Query(value = "select e.email" + " from Employee e")
    List<String> findAllEmails();

    @Query(value = "select e.phoneNumber" + " from Employee e")
